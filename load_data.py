...
class Excel_dataset(Dataset):
    ...
    def __init__(self, dir, if_normalize=False, if_onehot=False):
        ...
        elif (dir.endswith('.xlsx') or dir.endswith('.xls')):  #load data to vir
            data = pd.read_excel(dir, engine="openpyxl")

        nplist = data.T.to_numpy() #transfer to np to split data and target
        data = nplist[0:-1].T
        self.data = np.float64(data)
        self.target = nplist[-1]

        self.target_type = []
        #recorde class of label
        for i in self.target:
            try:
                ...
                # print(i)  
        # Convert tags to natural number encoding
        self.target_num = []
        for i in self.target:
